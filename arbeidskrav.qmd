...
Kommentar til OLS modellen: **Koeffisienter**

• **Intercept**: Konstantleddet (-247,700) er ikke signifikant (p = 0.2206), så den kan ikke tolkes som meningsfull.
...
• **Bathrooms**: Koeffisienten (-5,285) er ikke signifikant (p = 0.651), så antall bad ser ikke ut til å ha noen klar effekt på prisen i denne modellen.
...
**Floors :** Antall etasjer har ingen signifikant effekt på prisen (p = 0.659), noe som tyder på at dette ikke er en viktig prisdriver i denne modellen.
...
**Andel svart alene**: Koeffisienten (-91,680) er ikke signifikant (p = 0.707), så det ser ikke ut til at denne variabelen har en klar effekt på prisene.
...
**Andel andre alene**: Denne koeffisienten (-396,200) er ikke signifikant (p = 0.107), men trenden ser ut til å være negativ, noe som antyder at det kan være en svak sammenheng mellom denne variabelen og lavere boligpriser.

**Andel blandet**: Denne variabelen ble ikke estimert på grunn av “singularities,” som kan skyldes at det er perfekt korrelasjon mellom denne variabelen og andre i modellen.
...
Kvadratfot boareal er den sterkeste driveren av boligprisene, mens variabler som antall etasjer og bad ikke ser ut til å spille en stor rolle.
...
**OLS-modellen**:
Moran-plot for OLS-residualene viser tydelig spatial autokorrelasjon.
